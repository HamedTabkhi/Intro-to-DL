# -*- coding: utf-8 -*-
"""Transformer-Encoder-Regression.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1hxWORPA3xVeRT4Mz_Ht8J4zzgA2Z0YKX
"""
...
class TransformerModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, n_layers, n_heads):
        ...
        self.input_emb = nn.Linear(input_size, hidden_size)  # Embedding layer to project input to hidden size
    ...
    def forward(self, x):
        x = self.input_emb(x)  # Transform input to hidden size
        x = self.transformer_encoder(x)
        x = self.fc(x[:, -1, :])  # Output from the last time step
        return x

# Model parameters
input_size = 1
hidden_size = 128  # Can be set based on system capabilities and the complexity needed
output_size = 1
n_layers = 2
n_heads = 2
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)  # Changed optimizer to Adam for better performance
...
plt.savefig('./sine_wave_prediction_transformer.png')  # Save the plot as a file